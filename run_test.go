...
func TestRunAttemptsToListenForMixedHTTPAndWebhookPipelines(t *testing.T) {
	t.Setenv("OUVRIER_STATE_BACKEND", "memory")
	...
	if errors.Is(err, ovr.ErrRunNotImplemented) {
		t.Fatalf("Run error = %v, no longer want ErrRunNotImplemented for mixed HTTP/Webhook pipelines", err)
	}
	if !strings.Contains(err.Error(), "listen") {
		t.Fatalf("Run error = %v, want listen context", err)
	}
